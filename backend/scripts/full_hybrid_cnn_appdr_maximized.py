...
OUTPUT_DIR = BACKEND_DIR / "results" / "full_hybrid_cnn_appdr_maximized"
...
def parse_cnn_specs(text: str) -> list[dict[str, Any]]:
    ...
    for raw in [part.strip() for part in text.split(",") if part.strip()]:
        ...
        source_id = f"{backbone}_{input_size}_{objective}_gem"
...
def train_one_cnn_source(spec: dict[str, Any], manifests: dict[str, pd.DataFrame], args: argparse.Namespace) -> dict[str, Any]:
    ...
    for epoch in range(1, args.cnn_epochs + 1):
        ...
        if val_macro_f1 > best_val:
            ...
            torch.save({"state_dict": best_state, "spec": spec, "pretrained": model.pretrained}, checkpoint_path)
        else:
            stale += 1
    ...
    write_json(source_dir / "config.json", {**spec, "epochs_requested": args.cnn_epochs, "batch_size": args.cnn_batch_size, "pretrained": bool(getattr(model, "pretrained", False))})
...
def write_comparison_reports(best_five: ModelResult, best_binary: ModelResult, best_threshold: dict[str, Any]) -> None:
    ...
    (OUTPUT_DIR / "maximized_hybrid_vs_appdr_report.md").write_text(report["markdown"], encoding="utf-8")
    write_json(OUTPUT_DIR / "maximized_hybrid_vs_appdr_report.json", report)
